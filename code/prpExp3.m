...
% en el segundo experimento, el autor usa tres sujetos, asi que aqui
% se puede cargar datos de sujetos individuales y al final obtener el 
% desempeno de un sujeto. 
sOrder=[1 2 3 4 5];
keepPct = 0.22;

for k=1:5
    subject=['S0' num2str(sOrder(1))];
    [Xi, yi] = prpGetDataExp3(subject, keepPct);
    xTrainImages=Xi; tTrain=yi;
    
    subject=['S0' num2str(sOrder(2))];
    [Xi, yi] = prpGetDataExp3(subject, keepPct);
    xTrainImages=cat(2,xTrainImages,Xi); tTrain=cat(2,tTrain,yi);
    
    subject=['S0' num2str(sOrder(3))];
    [Xi, yi] = prpGetDataExp3(subject, keepPct);
    xTrainImages=cat(2,xTrainImages,Xi); tTrain=cat(2,tTrain,yi);
    
    subject=['S0' num2str(sOrder(4))];
    [Xi, yi] = prpGetDataExp3(subject, keepPct);
    xTrainImages=cat(2,xTrainImages,Xi); tTrain=cat(2,tTrain,yi);
    ...
    subject=['S0' num2str(sOrder(5))];
    [xTestImages, tTest] = prpGetData(subject);
    ...
    autoenc1 = trainAutoencoder(xTrainImages,hiddenSize1, ...
        ...
        'ScaleData', false);
    save(['exp3-k-' num2str(k) '-' subject '-autoenc1.mat'], 'autoenc1');
    ...
    autoenc2 = trainAutoencoder(feat1,hiddenSize2, ...
        ...
        'ScaleData', false);
    save(['exp3-k-' num2str(k) '-' subject '-autoenc2.mat'], 'autoenc2');
    ...
    save(['exp3-k-' num2str(k) '-' subject '-softnet.mat'], 'softnet');
    ...
    save(['exp3-k-' num2str(k) '-' subject '-deepnet.mat'], 'deepnet');
    ...
    % error en test set
    yTe = deepnet(xTest);
    fprintf(['Testing Error on Subject ' subject '\n']);
    tesMtr = prpGetMetrics(yTe, tTest);
    plotconfusion(tTest,yTe);

    save(['exp3-k-' num2str(k) '-' subject '-Metr.mat'], '*Mtr', 'tTest', 'yTe');
    
    outFullTe = cat(2,yTe',tTest');
    dlmwrite(['exp3-' subject '-OutTe.csv'],outFullTe,'delimiter',',','-append');

    clear *Mtr x* t* softnet auto* feat* outFullTe yTe yTr deepnet

    sOrder = circshift(sOrder,1);
